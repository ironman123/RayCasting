...
******************************************************************************************/
#define ROUNDNUM(x) (static_cast<int>(x + 0.5f))
...
void Graphics::DrawRectOutLine(int x0, int y0, int width, int height, Color c)
{
...
void Graphics::DrawCircle(const Vei2& p, int radius, Color c)
{
	...
	{
		for (int x_loop = p.x - radius; x_loop < p.x + radius; x_loop++)
		{
			const int distSq = ((p.x - x_loop) * (p.x - x_loop)) + ((p.y - y_loop) * (p.y - y_loop));
...
void Graphics::DrawCircleOutline(const Vei2& p, int radius, Color c)
{
	...
	{
		for (int x_loop = p.x - radius; x_loop < p.x + radius + 1; x_loop++)
		{
			const int distSq = ((p.x - x_loop) * (p.x - x_loop)) + ((p.y - y_loop) * (p.y - y_loop));
			if (distSq >= radSq - radius && distSq <= radSq + radius)
			{
				PutPixel(x_loop, y_loop, c);
			}
		}
